...
```{r}
# Filter
seurat_obj %>% filter(groups == "g1")

# Select
seurat_obj %>% select(cell, nCount_RNA , groups)
...
# Bind datasets
seurat_obj %>% bind_rows(pbmc_seurat)
```
...
```{r}
seurat_obj %>% bind_rows(seurat_obj)
```
...
We can visualise using Seurat's visualisaiton functions. 
